...
def plot_conservation(T_taxa,conservation_score_fn,alignment_fn):
    ##################
    #plot conservation
    ##################
    x_values = []
    ...
    ####################
    y_values = np.empty((66,731))
    os.chdir("./plot_cordinates")
    index = 0
    ...
    os.chdir("../")
    plt.legend()
    plt.title('Comparison between conservation score and DELPHI consensus results on SH2 proteins')
    plt.xlabel('Amino acid residue index')
    plt.ylabel('Scores')
    plt.savefig("plot_consensus/consensus.pdf")
    plt.close()
...
def main():
    ...
    plot_conservation(T_taxa,conservation_score_fn,alignment_fn)
